  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        // ASTAR ID wei zhongdian
        int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
        // kongzhidian astar guiji 中点 向量与控制点切向量的点乘
        ...
        {//astar 平面法向量，与vij的点积，如果小于o,则需要向前移动id否则向后，直到找到切换点
          ...
          {// 如果本次和上次正负号不一样，同时不全为0， 根据投影关系计算交点
            intersection_point =
        ...
        {// got intersection
          ...
          {// have length
            ...
            { // check each grid cell
              ...
              {// if occ or reach ctrl pts
                if (occ)
                  a += grid_map_->getResolution();
                // set base point as the last not occ cell
      ...
      {// if too short seg, law is the line from end to first
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {// judge point is set to middle from start to end 
          last_Astar_id = Astar_id;
          ...
          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
          {
            ...
            {// set first to base, direction is intersec to middle
      ...
      //step 3
      if (got_intersection_id >= 0)
      {// from LAST intersection to end ctrl pts
        for (int j = got_intersection_id + 1; j <= final_segment_ids[i].second; ++j)
          if (!cps_.flag_temp[j])
          {// if no pairs， SET THE BASE POINT AS LAST PV PAIRS
